...
class Bottom_up():
    def __init__(self, model_name, batch_size, num_classes, dataset, u_data, save_path, load_path, embeding_fea_size=1024,
    ...
    def train(self, train_data, step, loss, dropout=0.5):
        ...
        trainer = Trainer(model, self.criterion, fixed_layer=self.fixed_layer)
        for epoch in range(epochs):
            adjust_lr(epoch, step_size)
            trainer.train(epoch, dataloader, optimizer, print_freq=300)#print_freq=max(5,len(dataloader) // 30 * 10))

        torch.save(model.state_dict(), osp.join(self.save_path, "classification_step_{}.ckpt".format(step)))
        self.model = model

    def resume(self, ckpt_file, step):
        print("continued from step", step)
        model = models.create(self.model_name, dropout=self.dropout, embeding_fea_size=self.embeding_fea_size, fixed_layer=self.fixed_layer)
    ...
    def evaluate(self, query, gallery, step):
        _load_path = osp.join(self.load_path, 'features_{}_step{}.pickle'.format(self.name, step))
        test_loader = self.get_dataloader(list(set(query) | set(gallery)), training=False)
        evaluator = Evaluator(self.model)
        rank1, mAP = evaluator.evaluate(test_loader, _load_path, query, gallery)
        return rank1, mAP
